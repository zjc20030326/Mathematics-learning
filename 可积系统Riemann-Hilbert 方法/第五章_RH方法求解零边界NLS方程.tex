...
\begin{callout}[编号说明]
    本笔记直接从原书第 5 章进入，不再单独设置``前四章基础''章节。凡是用到前面章节的结果，或第一次出现但本章没有解释的概念，都放入 callout 补充框中。
    本笔记直接沿用原书的节号与公式编号，例如 \eqref{eq:5.1.15} 就是原书 \eqref{eq:5.1.15}；正文中的公式引用全部使用这个编号。第 0 节（主线导览）和第 5 节（逻辑闭环）是笔记自加的两节，原书没有对应内容。
\end{callout}

\section*{先把整条主线理清楚}

书中以零边界聚焦 NLS 为例，但真正要掌握的是一类具有 Lax 对的可积方程如何转化成 RH 问题。整条路线不是``直接解非线性 PDE''，而是把非线性问题转移到谱平面上处理。
...
\begin{enumerate}[(1)]
    ...
    \item \textbf{Lax 对：把 NLS 编码成线性谱问题}
          ...
          \textbf{为什么走到下一步：}
          两个线性方程不能彼此矛盾，相容条件 $\Psi_{xt}=\Psi_{tx}$ 给出零曲率方程，并恰好恢复原来的非线性 PDE。
          零边界下 $q(x,t)\to0$，所以 $x\to\pm\infty$ 时系统退化成可以显式求解的自由系统。

          \Needspace{8\baselineskip}
    \item \textbf{Jost 解：用两个无穷远的自由渐近条件挑出基本解}

          Jost 解在 $x\to-\infty$ 与 $x\to+\infty$ 分别与自由波匹配；再把已知的指数振荡因子提出，得到趋于单位矩阵的归一化特征函数 $\mu_1,\mu_2$。

          \textbf{为什么走到下一步：}
          整条实轴有两个归一化端点，所以有两个 Jost 解；剥掉自由振荡以后，$\mu_j$ 才适合讨论谱平面上的解析性和 $z\to\infty$ 的规范化。

          \Needspace{8\baselineskip}
    \item \textbf{等价的 Lax 对}

          把 $\mu_j=\psi_j e^{i\theta\sigma_3}$ 代回 Lax 对，得到等价的线性系统
          ...
          \textbf{为什么走到下一步：}
          这个形式里的未知函数趋于 $I$，且不再含已知的指数因子，可以直接对谱参数 $z$ 作展开。

          \Needspace{8\baselineskip}
    \item \textbf{无穷远点 Taylor 展开：先建立重构公式}

          \[
              \mu
              =I+\dfrac{\mu^{(1)}}{z}+O(z^{-2}),
              \qquad
              q(x,t)
              =2i(\mu^{(1)})_{12}
          ...
          \textbf{为什么走到下一步：}
          这一步先拿到``输出接口''——只要能从后面的 RH 问题解出 $\mu$ 在 $z=\infty$ 处的首个非平凡系数，就恢复出 $q(x,t)$。

          \Needspace{8\baselineskip}
    \item \textbf{全微分形式：得到两个特征函数与谱矩阵函数}

          把第 (3) 步写成全微分

          \[
              \mathrm{d}\!\left(e^{i\theta\widehat{\sigma}_3}\mu\right)
              =
              e^{i\theta\widehat{\sigma}_3}
              \left[(P\,\mathrm{d}x+Q\,\mathrm{d}t)\mu\right].
          \]

          沿 $x$ 从左右无穷远积分，得到两个特征函数 $\mu_1,\mu_2$（即两个 Volterra 积分方程）；它们作为同一个 Lax 对的基本矩阵解，彼此相差一个与 $x,t$ 无关的矩阵

          \[
              \psi_1=\psi_2S(z).
          \]

          $S(z)$ 就是谱矩阵（散射矩阵），它把初值 $q_0$ 编码成谱数据。

          \textbf{为什么走到下一步：}
          积分核里出现 $e^{\pm2iz(x-y)}$，它在上下半平面的衰减性直接决定 $\mu_1,\mu_2$ 各列、各行的解析区域。

          \Needspace{8\baselineskip}
    \item \textbf{解析性拼接：拼出 $P_+,P_-$，得到 RH 问题}

          把上半平面解析的列、下半平面解析的行分别拼起来，得到 $P_+,P_-$，于是

          \[
              P_-(x,t,z)P_+(x,t,z)=G(x,t,z),
              \qquad z\in\mathbb{R}.
          \]

          再配上两侧的解析性和无穷远归一化 $P_\pm\to I$，就是一个完整的 RH 问题；重构公式相应地变成
          ...
          \textbf{为什么走到下一步：}
          原来的 PDE 已经被完全转化成一个复平面边值问题，接下来只需要讨论这个 RH 问题是否正则、怎么求解。

          \Needspace{8\baselineskip}
    \item \textbf{区分正则与非正则 RH 问题}

          \begin{itemize}
              \item 无离散零点：正则 RH，用 Plemelj（普莱梅尔）公式转成奇异积分方程；
              \item 有离散零点：非正则 RH，用 $\Gamma(z)$ 把零点结构剥离，再化成正则 RH。
          \end{itemize}

          \textbf{为什么走到下一步：}
          求出 RH 解以后，用第 (6) 步的重构公式恢复 $q(x,t)$。

          \textbf{无反射情形：}
          连续谱跳跃消失，只剩离散谱，RH 问题退化成有限维代数问题，从而得到 $N$ 孤子解。
...
\begin{callout}[前置概念：Schwartz 空间与``固定 $t$ 时仍然零边界''（原书定义 4.4，4.1.2 节）]
    Schwartz 空间定义为
    ...
    \[
        ...
        \sup_{x\in\mathbb{R}}\lvert x^\alpha\partial^\beta f(x)\rvert<\infty,\
        \alpha,\beta\in\mathbb{Z}_+
        \right\}.
    \]
    它表示函数及其任意阶导数都快于任意多项式幂衰减。
    仅由 $q_0\in\mathcal S$ 的定义，不能直接推出所有时刻都衰减；这里还默认使用了 NLS 流保持 Schwartz 正则性这一结论：在所考虑的光滑解范围内，对每个固定的有限 $t$，有 $q(\cdot,t)\in\mathcal S(\mathbb{R})$。
    因此

    \[
        q(x,t)\to0,\qquad \lvert x\rvert\to\infty.
    \]
    这里的``零边界''指空间无穷远处的边界为零，并不是 $q\equiv0$。
\end{callout}
...
\begin{callout}[概念：Jost 解为什么有左右两个？]
    ...
    \]
    因此``两个 Jost 解''不是说线性系统只有两个解，而是说我们选取了两个不同无穷远归一化条件下的基本矩阵解。
\end{callout}
...
\begin{callout}[概念：$\widehat{\sigma}_3$ 是什么？]
    带帽的 $\widehat{\sigma}_3$ 不是 $2\times2$ 矩阵，而是作用在矩阵空间上的交换子算子：
...
其中 $\mu^{(0)},\mu^{(1)},\ldots$ 与 $z$ 无关。
将 \eqref{eq:5.1.10} 代入 Lax 对 \eqref{eq:5.1.7}---\eqref{eq:5.1.8}，比较 $z$ 的幂次系数，可得到 $x$-部分：
...
由 \eqref{eq:5.1.11}、\eqref{eq:5.1.13}，知道 $\mu^{(0)}$ 为对角矩阵，而 \eqref{eq:5.1.12}、\eqref{eq:5.1.14} 推知 $\mu_x^{(0)}=0$。
这说明 $\mu^{(0)}$ 是与 $x$ 无关的对角矩阵。
因此 \eqref{eq:5.1.10} 对 $x,z$ 同时取极限，并注意交换极限顺序，我们有

\[
    \lim\limits_{z\to\infty}\lim\limits_{\lvert x\rvert\to\infty}\mu
    =
    \lim\limits_{\lvert x\rvert\to\infty}\lim\limits_{z\to\infty}\left(\mu^{(0)}+O(z^{-1})\right).
\]

利用 \eqref{eq:5.1.6} 和 \eqref{eq:5.1.10}，上述左边极限为 $I$，右边为 $\mu^{(0)}$，因此有 $\mu^{(0)}=I$，从而

\[
    \mu\to I,\qquad z\to\infty.
\]

再将 $\mu^{(0)}=I$ 代回 \eqref{eq:5.1.14}，比较矩阵对应元素，可得到
...
又因为 $\mu_j=\psi_je^{i\theta\sigma_3}$ 且 $\det e^{i\theta\sigma_3}=1$，所以 $\det\mu_j=\det\psi_j$。结合空间无穷远归一化，
...
由正则部分的 Plemelj 表示作 $z\to\infty$ 展开，有
...
时，由 \eqref{eq:5.3.7} 有 $G=I$，连续谱积分项消失：
...
这里 $M^*$ 在这一式中表示 \textbf{伴随矩阵（adjugate）}，不是 Hermite 共轭。使用
